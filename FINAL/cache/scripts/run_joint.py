...
def train_phase(complexes, events, pw, macro=None, resume=False, add_rounds=20):
    """联合训练,返回模型列表。进度 0-50%。resume=True 时为增量模式。"""
    last_pct = [0]
    ...
    if resume and os.path.exists(MODEL_FILE):
        try:
            ...
            new_n = len(complexes)
            report(f"已有模型: {old_n} 个小区,当前 {new_n} 个(新增 {new_n - old_n} 个)", 2)
        except Exception as e:
            print(f"  加载已有模型失败({e}),将从零开始")
            existing = None
    ...
    if resume and existing and "models" in existing:
        ...
        merged_rounds = existing.get("n_rounds", 0) + n_rounds
    else:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="联合训练 + 自动预测")
    ...
    run_with_progress(lambda pw: _run(pw, resume=args.resume, add_rounds=args.add_rounds),
                      _make_progress, title="增量训练运行中" if args.resume else "联合训练运行中")


def _run(pw, resume=False, add_rounds=20):
    def report(text, pct, log=None):
        if pw is not None:
            pw.update(text, pct, log)
        else:
